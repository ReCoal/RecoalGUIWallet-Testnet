...
class ElectroneumdManager(ProcessManager):
    def __init__(self, resources_path, log_level=0, block_sync_size=10):
        ...
        proc_args = u'%s/bin/recoald --log-level %d --block-sync-size %d --rpc-bind-port %d --testnet' % (resources_path, log_level, block_sync_size, RPC_DAEMON_PORT)
...
class WalletCliManager(ProcessManager):
    ...
    def __init__(self, resources_path, wallet_file_path, wallet_log_path, restore_wallet=False):
        ...
        if not restore_wallet:
            wallet_args = u'%s/bin/recoal-wallet-cli --generate-new-wallet=%s --log-file=%s --testnet' \
                                                % (resources_path, wallet_file_path, wallet_log_path)
        else:
            wallet_args = u'%s/bin/recoal-wallet-cli --log-file=%s --restore-deterministic-wallet --testnet' \
...
class WalletRPCManager(ProcessManager):
    def __init__(self, resources_path, wallet_file_path, wallet_password, app, log_level=2):
        ...
        wallet_file_path  = wallet_file_path.replace(' ' ,'__SPACE_REPLACE__')
        wallet_log_path = wallet_log_path.replace(' ' ,'__SPACE_REPLACE__')
        wallet_rpc_args = u'%s/bin/recoal-wallet-rpc --testnet --disable-rpc-login --wallet-file %s --log-file %s --rpc-bind-port %d --log-level %d --daemon-port %d --password %s' % (resources_path, wallet_file_path, wallet_log_path, RPC_DAEMON_PORT+2, log_level, RPC_DAEMON_PORT, wallet_password)
